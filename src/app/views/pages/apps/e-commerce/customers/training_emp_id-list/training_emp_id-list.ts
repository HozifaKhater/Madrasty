...
import { divisions, divisionsMaster} from '../../../../../../divisionsMaster.Model';
...
@Component({
    ...
    selector: 'kt-training_emp-list',
    templateUrl: './training_emp_id-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class training_empListComponent implements OnInit, OnDestroy {
	// Table fields
	classes: Classes[];
	selectedclass: any;
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
    displayedColumns = ['select', 'trainig_id', 'trainig_name','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Twze3_studentsMaster: Twze3_studentsMaster[];
	...
	filterType: string = '';
	// Selection
    selection = new SelectionModel<divisions>(true, []);
	...
	constructor(
		...
		private Twze3_studentsDataService: Twze3_studentsDataService,
		private ClassesDataService: ClassesDataService,
		private AbsenceDataService: AbsenceDataService,
        private divisionsDataService: divisionsDataService,
        private basic_dataDataService: basic_dataDataService

	) {
		//this.get_Twze3_students(this.StudentDataService);
		this.ClassesDataService.GetAllClasses().subscribe(data => this.classes = data,
			error => console.log(error),
			() => console.log("emp dropdown", this.classes));
		this.dataSource = new MatTableDataSource([])
		//this.StudentDataService.GetAllstudents_with_id(3).subscribe(data => this.ELEMENT_DATA = data,
		//	error => console.log(error),
		//	() => { this.dataSource = new MatTableDataSource(this.ELEMENT_DATA), console.log("ffff", this.ELEMENT_DATA) }
		//);
	}
	get_Twze3_students() {

        this.basic_dataDataService.GetAlltraining_course_with_emp_id(this.basic_dataDataService.emp_id).subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(error),
			() => { this.dataSource = new MatTableDataSource(this.ELEMENT_DATA), console.log("ffff", this.ELEMENT_DATA) }
		); }
	/**
	...
	//departmentemp(event) {
	//	this.Twze3_studentsDataService.update_class_in_student(student_id).subscribe(data => this.Classes = data,
	//		error => console.log(error),
	//		() => console.log("emp dropdown", this.Classes));
	//}
    ngAfterViewInit() {
        this.basic_dataDataService.bClickedEvent
            .subscribe((data: string) => {
                console.log("edited");
                this.get_Twze3_students();});
 ...
 //       for (let i = 0; i < this.selection.selected.length; i++) {
 //           console.log('wwrd', this.selection.selected, this.selection.selected[i].student_class_id.class_id
 //               , this.selection.selected[i].student_class_id.class_name);
 //           var val = {

 //               student_id: Number(this.selection.selected[i].student_id ),
 //               student_class_id: Number(this.selection.selected[i].student_class_id.class_id),
 //               student_class_name: this.selection.selected[i].student_class_id.class_name 
 //       };
 //           console.log("val", val);
 //           this.Twze3_studentsDataService.update_class_in_student(val).subscribe(res => {
 //               console.log("updated!");
 //               this.get_Twze3_students(this.StudentDataService)
 ...
 //       //for (let i = 0; i < this.selection.selected.length; i++) {
	...
	//			//	var val = {
	//			//		student_id: this.Twze3_studentsDataService.student_id,
	//			//		student_class_id: this.Twze3_studentsDataService.student_class_id,
	//			//		student_class_name: this.Twze3_studentsDataService.student_class_name
	//			//	};
	//			//	this.Twze3_studentsDataService.update_class_in_student(val).subscribe(res => { console.log("val", val) })
	//			//}
	...
	twze3_students_info: any [];
	editCustomer(customer: Twze3_students, Twze3_studentsDataService: Twze3_studentsDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer.twze3_id);
		this.Twze3_studentsDataService.GetAllTwze3_students_with_id(customer.twze3_id).subscribe(data => this.twze3_students_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.twze3_students_info) {
					console.log(item.Evaluation_item_id)	
					this.Twze3_studentsDataService.twze3_id = Number(item.twze3_id);
					console.log('testdepname', item.Evaluation_item_name);
					
				
					this.Twze3_studentsDataService.twze3_lev = item.twze3_lev;
					this.Twze3_studentsDataService.student_id = item.student_id;
					this.Twze3_studentsDataService.student_name = item.student_name;
					this.Twze3_studentsDataService.class_id = item.class_id;
					this.Twze3_studentsDataService.class_name = item.class_name;
			

	
				};
				console.log('Component A is clicked!!', this.Twze3_studentsDataService.student_name);
				this.Twze3_studentsDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Twze3_students, Twze3_studentsDataService: Twze3_studentsDataService) {
	
		console.log('CUSTOMER ID', customer.twze3_id);
		this.Twze3_studentsDataService.deleteTwze3_students(Number(customer.twze3_id)).subscribe(res => {
			//this.get_Twze3_students(Twze3_studentsDataService);
			alert(res.toString());
