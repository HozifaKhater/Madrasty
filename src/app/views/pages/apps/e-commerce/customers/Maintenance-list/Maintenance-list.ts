...
import { MaintenanceMaster, Maintenance } from '../../../../../../MaintenanceMaster.Model';
...
@Component({
	...
	selector: 'kt-maint-list',
	templateUrl: './Maintenance-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class MaintListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'maint_id', 'maint_type', 'maint_loc', 'maint_work','maint_notes','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_maint(MaintenanceDataService);

	}
	get_maint(MaintenanceDataService: MaintenanceDataService) {

		this.MaintenanceDataService.GetAllMaintenance().subscribe(data => this.ELEMENT_DATA = data,
	...
	Maint_info: any [];
	editCustomer(customer: Maintenance, MaintenanceDataService: MaintenanceDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.MaintenanceDataService.GetAllMaintenance_with_id(customer.maint_id).subscribe(data => this.Maint_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Maint_info) {
					console.log(item.maint_id)
					this.MaintenanceDataService.maint_id = Number(item.maint_id);
					console.log('testdepname', item.delay_emp_name);
					
					this.MaintenanceDataService.maint_date = item.maint_date;
					this.MaintenanceDataService.maint_type = item.maint_type;
					this.MaintenanceDataService.maint_loc = item.maint_loc;
					this.MaintenanceDataService.maint_work = item.maint_work;
					this.MaintenanceDataService.maint_notes = item.maint_notes;
				};
				console.log('Component A is clicked!!', this.MaintenanceDataService.maint_id);
				this.MaintenanceDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Maintenance, MaintenanceDataService: MaintenanceDataService) {
	
		console.log('CUSTOMER ID', customer.maint_id);
		this.MaintenanceDataService.deleteMaintenance(Number(customer.maint_id)).subscribe(res => {
			this.get_maint(MaintenanceDataService);
			alert(res.toString());
