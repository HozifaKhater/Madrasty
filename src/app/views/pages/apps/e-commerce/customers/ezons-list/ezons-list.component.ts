...
export class EzonListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'ezn_id', 'emp_name', 'ezn_date', 'time_from', 'time_to','ezn_state', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_ezons(EzonDataService) {
		EzonDataService.GetAllEzon().subscribe(data => this.ELEMENT_DATA = data,
	...
	deleteCustomers() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			for (let i = 0; i < this.selection.selected.length; i++) {
				...
				console.log("delete_customers", this.selection.selected[i])
