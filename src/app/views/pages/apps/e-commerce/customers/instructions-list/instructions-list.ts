...
@Component({
    ...
    selector: 'kt-instructions-list',
    templateUrl: './instructions-list.html',
...
export class instructionsListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
    displayedColumns = ['select', 'ser', 'group_number', 'notes','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_delays() {
        ...
        this.instructionsDataService.GetAllinstructions().subscribe(data => this.ELEMENT_DATA = data,
	...
	Delays_info: any [];
    editCustomer(instructions: instructions ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
        console.log('CUSTOMER ID', instructions);
        this.instructionsDataService.ser = Number(instructions.ser);
        this.instructionsDataService.GetAllinstructions_with_id(instructions.ser).subscribe(data => this.Delays_info = data,
			...
			() => {
				for (let item of this.Delays_info) {
                    ...
                    this.instructionsDataService.level_name = item.level_name;
                    this.instructionsDataService.level_id = item.level_id;
                    this.instructionsDataService.class_name = item.class_name;
                    this.instructionsDataService.class_id = item.class_id;
                    this.instructionsDataService.topic = item.topic;
                    this.instructionsDataService.group_number = item.group_number;
                    this.instructionsDataService.sessions_number = item.sessions_number;
                    this.instructionsDataService.notes = item.notes;


		
				};
                console.log('Component A is clicked!!', this.instructionsDataService.ser);
                this.instructionsDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
    deleteCustomer(instructions: instructions) {
	
        console.log('CUSTOMER ID', instructions.ser);
        this.instructionsDataService.deleteinstructions(Number(instructions.ser)).subscribe(res => {
			this.get_delays();
			alert(res.toString());
