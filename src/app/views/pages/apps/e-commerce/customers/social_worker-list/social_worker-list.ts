...
import { Social_workerDataService } from '../../../../../../Services/Social_workerDataService';

import { Social_workerMaster, Social_worker } from '../../../../../../Social_workerMaster.Model';
...
@Component({
	...
	selector: 'kt-social_worker-list',
	templateUrl: './social_worker-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class social_workerListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'social_id', 'social_students', 'social_report', 'social_man_dep', 'social_super',  'social_worker_opin', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Social_workerMaster: Social_workerMaster[];
	...
	constructor(
		...
		private Social_workerDataService: Social_workerDataService
	) {
		this.get_social_worker(Social_workerDataService);

	}
	get_social_worker(Social_workerDataService: Social_workerDataService) {

		this.Social_workerDataService.GetAllSocial_worker().subscribe(data => this.ELEMENT_DATA = data,
	...
	Social_info: any [];
	editCustomer(customer: Social_worker, Social_workerDataService: Social_workerDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.Social_workerDataService.GetAllSocial_worker_with_id(customer.social_id).subscribe(data => this.Social_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Social_info) {
					console.log(item.social_id)
					this.Social_workerDataService.social_id = Number(item.social_id);
					console.log('testdepname', item.social_level);
					
					this.Social_workerDataService.social_level = item.social_level;
					this.Social_workerDataService.social_class = item.social_class;
					this.Social_workerDataService.social_dep = item.social_dep;
					this.Social_workerDataService.social_students = item.social_students;


					this.Social_workerDataService.social_date = item.social_date;
					this.Social_workerDataService.social_report = item.social_report;
					this.Social_workerDataService.social_man_dep = item.social_man_dep;
					this.Social_workerDataService.social_super = item.social_super;
					this.Social_workerDataService.social_worker_opin = item.social_worker_opin;
				};
				console.log('Component A is clicked!!', this.Social_workerDataService.social_id);
				this.Social_workerDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Social_worker, Social_workerDataService: Social_workerDataService) {
	
		console.log('CUSTOMER ID', customer.social_id);
		this.Social_workerDataService.deleteSocial_worker(Number(customer.social_id)).subscribe(res => {
			this.get_social_worker(Social_workerDataService);
			alert(res.toString());
