...
@Component({
	...
	selector: 'kt-group_meeting-list',
	templateUrl: './group-meeting-list.html',
...
export class GroupmeetingListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'group_id', 'group_name', 'meeting_no', 'meeting_date', 'meeting_mem_no','meeting_loc','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Group_meetingMaster: Group_meetingMaster[];
	...
	constructor(
		...
		this.get_groups(Group_meetingDataService);

	}
	get_groups(Group_meetingDataService: Group_meetingDataService) {

		this.Group_meetingDataService.GetAllGroup_meeting().subscribe(data => this.ELEMENT_DATA = data,
	...
	Groups_info: any [];
	editCustomer(customer: Group_meeting, Group_meetingDataService: Group_meetingDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.Group_meetingDataService.GetAllGroup_meeting_with_id(customer.group_id).subscribe(data => this.Groups_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Groups_info) {
					console.log(item.group_id)
					this.Group_meetingDataService.group_id = Number(item.group_id);
					console.log('testdepname', item.group_id);
	...
	deleteCustomer(customer: Group_meeting, Group_meetingDataService: Group_meetingDataService) {
	
		console.log('CUSTOMER ID', customer.group_id);
		this.Group_meetingDataService.deleteGroup_meeting(Number(customer.group_id)).subscribe(res => {
			this.get_groups(Group_meetingDataService);
			alert(res.toString());
