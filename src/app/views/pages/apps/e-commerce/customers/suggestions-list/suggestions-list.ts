...
@Component({
	...
	selector: 'kt-sugges-list',
	templateUrl: './suggestions-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class SuggesListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'sugg_id', 'sugg_type', 'sugg_title', 'sugg_file', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_sugges(SuggestionsDataService);

	}
	get_sugges(SuggestionsDataService: SuggestionsDataService) {

		this.SuggestionsDataService.GetAllSuggestions().subscribe(data => this.ELEMENT_DATA = data,
	...
	Sugg_info: any [];
	editCustomer(customer: Suggestions, SuggestionsDataService: SuggestionsDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.SuggestionsDataService.GetAllSuggestions_with_id(customer.sugg_id).subscribe(data => this.Sugg_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Sugg_info) {
					console.log(item.sugg_id)
					this.SuggestionsDataService.sugg_id = Number(item.sugg_id);
					console.log('testdepname', item.sugg_title);
					
					this.SuggestionsDataService.sugg_body = item.sugg_body;
					this.SuggestionsDataService.sugg_title = item.sugg_title;
					this.SuggestionsDataService.sugg_file = item.sugg_file;
					this.SuggestionsDataService.sugg_type = item.sugg_type;

		
				};
				console.log('Component A is clicked!!', this.SuggestionsDataService.sugg_id);
				this.SuggestionsDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Suggestions, SuggestionsDataService: SuggestionsDataService) {
	
		console.log('CUSTOMER ID', customer.sugg_id);
		this.SuggestionsDataService.deleteSuggestions(Number(customer.sugg_id)).subscribe(res => {
			this.get_sugges(SuggestionsDataService);
			alert(res.toString());
