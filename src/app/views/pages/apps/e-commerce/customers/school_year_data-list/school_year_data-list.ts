...
@Component({
	...
	selector: 'kt-school_year_data-list',
    templateUrl: './school_year_data-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class SchoolYearListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
    displayedColumns = ['select', 'year_data_id', 'year_date_from','year_date_to', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
    ...
    School_year_dataMaster: School_year_dataMaster[];
	...
	constructor(
        ...
        this.get_year(School_year_dataDataService);

	}
    get_year(NchraDataService: School_year_dataDataService) {

        this.School_year_dataDataService.GetAllSchool_year_data().subscribe(data => this.ELEMENT_DATA = data,
	...
	year_info: any [];
    editCustomer(year: School_year_data, School_year_dataDataService: School_year_dataDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
        console.log('CUSTOMER ID', year,Number(year.year_data_id));
        this.School_year_dataDataService.year_data_id = Number(year.year_data_id);
        this.School_year_dataDataService.GetAllSchool_year_data_with_id(year.year_data_id).subscribe(data => this.year_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.year_info) {
                   // console.log(item.year_data_id)
					//this.NchraDataService.nchra_id = Number(item.nchra_id);
			//console.log('testdepname', item.nchra_sender);
					
                    this.School_year_dataDataService.year_date_from = item.year_date_from;
                    this.School_year_dataDataService.year_date_to = item.year_date_to;
                  
					

		
				};
                console.log('Component A is clicked!!', this.School_year_dataDataService.year_data_id);
                this.School_year_dataDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
    deleteCustomer(year: School_year_data, School_year_dataDataService: School_year_dataDataService ) {
	
        console.log('CUSTOMER ID', year.year_data_id);
        this.School_year_dataDataService.deleteSchool_year_data(Number(year.year_data_id)).subscribe(res => {
            this.get_year(School_year_dataDataService);
			alert(res.toString());
