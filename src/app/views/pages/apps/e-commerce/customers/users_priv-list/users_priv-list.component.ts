...
export class users_privComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'user_id', 'emp_name', 'job_name', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_users_priv(user_privDataService);
	}
	get_users_priv(user_privDataService) {
		this.user_privDataService.GetAlluser_privs().subscribe(data => this.ELEMENT_DATA = data,
	...
	delete_corridor(users_priv: users_priv, user_privDataService: user_privDataService ) {
		...
		console.log('CUSTOMER ID', users_priv.user_id);
		this.user_privDataService.deleteuser_privs(Number(users_priv.user_id)).subscribe(res => {
			this.get_users_priv(user_privDataService);;
			alert(res.toString());
