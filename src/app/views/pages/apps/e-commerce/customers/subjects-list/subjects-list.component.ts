...
export class SubjectComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'subject_id', 'subject_name', 'subject_desc', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	subjects_info: any[];
	edit_subject(Subjects: Subjects, DepartmentService: DepartmentDataService, SubjectService: SubjectDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('Subject ID', Subjects.subject_id);
		this.SubjectService.GetAllSubject_with_id(Subjects.subject_id).subscribe(data => this.subjects_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.subjects_info) {
					console.log(item.dep_id)
					this.SubjectService.subject_id = Number(item.subject_id);
					console.log('testsuname', item.subject_name);
					this.SubjectService.subject_name = item.subject_name;
					this.SubjectService.subject_desc = item.subject_desc;
				};
				console.log('Component A is clicked!!', this.SubjectService.subject_name);
				this.SubjectService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(Subjects: Subjects,customer: CustomerModel, DepartmentService: DepartmentDataService, SubjectService: SubjectDataService) {
