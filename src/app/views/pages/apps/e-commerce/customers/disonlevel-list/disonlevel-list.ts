...
@Component({
	...
	selector: 'kt-disonlevel-list',
	templateUrl: './disonlevel-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush

	/*,providers: [DepartmentDataService]*/
})
export class DisonlevelListComponent implements OnInit, OnDestroy {
	// Table fields


	Element1: [{ id: "1" },
		{ id: "2" }];
	displayedColumns = ['select', 'lev_id', 'lev_name', 'lev_class_no', 'actions'];
	...
	constructor(
		...
		this.get_levels(LevelsDataService);

	}
	get_levels(LevelsDataService: LevelsDataService) {

		this.LevelsDataService.GetAllLevels().subscribe(data => this.ELEMENT_DATA = data,
	...
	Levels_info: any[];
	editCustomer(customer: Levels, LevelsDataService: LevelsDataService) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.LevelsDataService.GetAllLevels_with_id(customer.lev_id).subscribe(data => this.Levels_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Levels_info) {
					console.log(item.delay_id)
					this.LevelsDataService.lev_id = Number(item.lev_id);
					console.log('testdepname', item.lev_name);

					this.LevelsDataService.lev_name = item.lev_name;
					this.LevelsDataService.lev_class_no = item.lev_class_no;
					this.LevelsDataService.lev_desc = item.lev_desc;



				};
				console.log('Component A is clicked!!', this.LevelsDataService.lev_id);
				this.LevelsDataService.AClicked('Component A is clicked!!');
			}
		);


	}
	deleteCustomer(customer: Levels, LevelsDataService: LevelsDataService) {

		console.log('CUSTOMER ID', customer.lev_id);
		this.LevelsDataService.deleteLevels(Number(customer.lev_id)).subscribe(res => {
			this.get_levels(LevelsDataService);
			alert(res.toString());
