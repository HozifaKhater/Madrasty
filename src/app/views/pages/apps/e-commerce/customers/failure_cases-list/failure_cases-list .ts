...
@Component({
	...
	selector: 'kt-failure-list',
	templateUrl: './failure_cases-list .html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class failureListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'fail_id', 'fail_student', 'fail_sub', 'fail_end_year', 'fail_sit','fail_recomm', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Failure_casesMaster: Failure_casesMaster[];
	...
	constructor(
		...
		this.get_failure(Failure_casesDataService);

	}
	get_failure(Failure_casesDataService: Failure_casesDataService) {

		this.Failure_casesDataService.GetAllFailure_cases().subscribe(data => this.ELEMENT_DATA = data,
	...
	Failure_info: any [];
	editCustomer(customer: Failure_cases, Failure_casesDataService: Failure_casesDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.Failure_casesDataService.GetAllFailure_cases_with_id(customer.fail_id).subscribe(data => this.Failure_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Failure_info) {
					console.log(item.fail_id)
					this.Failure_casesDataService.fail_id = Number(item.fail_id);
					console.log('testdepname', item.lev_name);

					this.Failure_casesDataService.fail_lev = item.fail_lev;
					this.Failure_casesDataService.fail_class = item.fail_class;
					this.Failure_casesDataService.fail_student = item.fail_student;
					this.Failure_casesDataService.fail_nation = item.fail_nation;
					this.Failure_casesDataService.fail_mob = item.fail_mob;
					this.Failure_casesDataService.fail_birth = item.fail_birth;
					this.Failure_casesDataService.fail_date = item.fail_date;
					this.Failure_casesDataService.fail_desc = item.fail_desc;
					this.Failure_casesDataService.fail_reason = item.fail_reason;
					this.Failure_casesDataService.fail_sub = item.fail_sub;
					this.Failure_casesDataService.fail_1 = item.fail_1;
					this.Failure_casesDataService.fail_2 = item.fail_2;
					this.Failure_casesDataService.fail_3 = item.fail_3;
					this.Failure_casesDataService.fail_4 = item.fail_4;
					this.Failure_casesDataService.fail_end_year = item.fail_end_year;
					this.Failure_casesDataService.fail_sit = item.fail_sit;
					this.Failure_casesDataService.fail_eff_date = item.fail_eff_date;
					this.Failure_casesDataService.fail_eff_results = item.fail_eff_results;
					this.Failure_casesDataService.fail_recomm = item.fail_recomm;


		
				};
				console.log('Component A is clicked!!', this.Failure_casesDataService.fail_id);
				this.Failure_casesDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Failure_cases, Failure_casesDataService: Failure_casesDataService) {
	
		console.log('CUSTOMER ID', customer.fail_id);
		this.Failure_casesDataService.deleteFailure_cases(Number(customer.fail_id)).subscribe(res => {
			this.get_failure(Failure_casesDataService);
			alert(res.toString());
