...
@Component({
	...
	selector: 'kt-basic_data-list',
    templateUrl: './basic_data-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class basic_dataListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
    displayedColumns = ['select', 'id', 'name', 'register_date','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_delays() {
        ...
        this.basic_dataDataService.GetAllbasic_data().subscribe(data => this.ELEMENT_DATA = data,
	...
	Delays_info: any [];
    editCustomer(basic_data: basic_data ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
        console.log('CUSTOMER ID', basic_data);
        this.basic_dataDataService.id = basic_data.id;
        this.basic_dataDataService.GetAllbasic_data_with_id(basic_data.id).subscribe(data => this.Delays_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Delays_info) {
				
		
                    this.basic_dataDataService.name = item.name;
                    this.basic_dataDataService.file_number = item.file_number;
                    this.basic_dataDataService.identity_number = item.identity_number;
                    this.basic_dataDataService.qualification = item.qualification;
                    this.basic_dataDataService.register_date = item.register_date;
                    this.basic_dataDataService.civil_number = item.civil_number;
                    this.basic_dataDataService.exp_years = item.exp_years;
                    this.basic_dataDataService.emp_id = item.emp_id;


		
				};
                console.log('Component A is clicked!!', this.basic_dataDataService.id);
                this.basic_dataDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
    deleteCustomer(basic_data: basic_data ) {
	
        console.log('CUSTOMER ID', basic_data.id);
        this.basic_dataDataService.deletebasic_data(Number(basic_data.id)).subscribe(res => {
            this.get_delays();
			alert(res.toString());
