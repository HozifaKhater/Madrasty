...
export class guideListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
    displayedColumns = ['select', 'id', 'type_name', 'student_name','services','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_delays() {
        ...
        this.guideDataService.GetAllguide().subscribe(data => this.ELEMENT_DATA = data,
	...
	Delays_info: any [];
	editCustomer(customer: guide ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
        console.log('CUSTOMER ID', customer);
        this.guideDataService.id = customer.id
		this.guideDataService.GetAllguide_with_id(customer.id).subscribe(data => this.Delays_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Delays_info) {
					
                    this.guideDataService.type_id = item.type_id;
                    this.guideDataService.type_name = item.type_name;
                    this.guideDataService.level_id = item.level_id;
                    this.guideDataService.level_name = item.level_name;
                    this.guideDataService.class_id = item.class_id;
                    this.guideDataService.class_name = item.class_name;
                    this.guideDataService.student_id = item.student_id;
                    this.guideDataService.student_name = item.student_name;
                    this.guideDataService.services = item.services;
                    this.guideDataService.ntoes = item.ntoes;



		
				};
                console.log('Component A is clicked!!', this.guideDataService.id);
                this.guideDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: guide) {
	
		console.log('CUSTOMER ID', customer.id);
        this.guideDataService.deleteguide(Number(customer.id)).subscribe(res => {
			this.get_delays();
			alert(res.toString());
