...
@Component({
    ...
    selector: 'kt-branch_stat-list',
    templateUrl: './branch_stat-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class branch_statComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
    displayedColumns = ['select','id', 'sho3ba', 'hady_34r', 'sany_34r','student_count'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_delays(DelaysDataService);

	}
	get_delays(DelaysDataService: DelaysDataService) {

        this.StudentDataService.get_branch_stistics().subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(error),
            () => { this.dataSource = new MatTableDataSource(this.ELEMENT_DATA); console.log("tettttt", this.ELEMENT_DATA) }
		); }
	/**
	...
	Delays_info: any [];
	editCustomer(customer: Delays, DelaysDataService: DelaysDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.DelaysDataService.GetAllDelays_with_id(customer.delay_id).subscribe(data => this.Delays_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Delays_info) {
					console.log(item.delay_id)
					this.DelaysDataService.delay_id = Number(item.delay_id);
					console.log('testdepname', item.delay_emp_name);
					
					this.DelaysDataService.delay_id = item.delay_id;
					this.DelaysDataService.delay_date = item.delay_date;
					this.DelaysDataService.delay_emp_name = item.delay_emp_name;
					this.DelaysDataService.time_attend = item.time_attend;
					this.DelaysDataService.delay_state = item.delay_state;

		
				};
				console.log('Component A is clicked!!', this.DelaysDataService.delay_id);
				this.DelaysDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Delays, DelaysDataService: DelaysDataService) {
	
		console.log('CUSTOMER ID', customer.delay_id);
		this.DelaysDataService.deleteDelays(Number(customer.delay_id)).subscribe(res => {
			this.get_delays(DelaysDataService);
			alert(res.toString());
