...
@Component({
	...
	selector: 'kt-mra7l-list',
	templateUrl: './mra7l-list .html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class Mra7lListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'mr7la_id', 'mr7la_name', 'mr7la_code','mr7la_desc', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		private Mra7lDataService: Mra7lDataService
	) {
		this.get_mra7l(Mra7lDataService);

	}
	get_mra7l(Mra7lDataService: Mra7lDataService) {

		this.Mra7lDataService.GetAllMra7l().subscribe(data => this.ELEMENT_DATA = data,
	...
	Mra7l_info: any [];
	editCustomer(customer: Mra7l, Mra7lDataService: Mra7lDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.Mra7lDataService.GetAllMra7l_with_id(customer.mr7la_id).subscribe(data => this.Mra7l_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Mra7l_info) {
					console.log(item.mr7la_id)
					this.Mra7lDataService.mr7la_id = Number(item.mr7la_id);
					console.log('testdepname', item.mr7la_name);
					
					this.Mra7lDataService.mr7la_name = item.mr7la_name;
					this.Mra7lDataService.mr7la_code = item.mr7la_code;
					this.Mra7lDataService.mr7la_desc = item.mr7la_desc;
			

		
				};
				console.log('Component A is clicked!!', this.Mra7lDataService.mr7la_id);
				this.Mra7lDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Mra7l, Mra7lDataService: Mra7lDataService) {
	
		console.log('CUSTOMER ID', customer.mr7la_id);
		this.Mra7lDataService.deleteMra7l(Number(customer.mr7la_id)).subscribe(res => {
			this.get_mra7l(Mra7lDataService);
			alert(res.toString());
