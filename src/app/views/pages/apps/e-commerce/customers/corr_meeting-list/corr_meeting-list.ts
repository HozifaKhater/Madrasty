...
@Component({
	...
	selector: 'kt-corr-meet-list',
	templateUrl: './corr_meeting-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class CorrMeetListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'corr_meet_id', 'corr_meet_date', 'corr_meet_time', 'corr_meet_loc','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Corr_meetingMaster: Corr_meetingMaster[];
	...
	constructor(
		...
		this.get_corr_meet(Corr_meetingDataService);

	}
	get_corr_meet(Corr_meetingDataService: Corr_meetingDataService) {

		this.Corr_meetingDataService.GetAllCorr_meeting().subscribe(data => this.ELEMENT_DATA = data,
	...
	Observ_info: any [];
	editCustomer(customer: Corr_meeting, Corr_meetingDataService: Corr_meetingDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.Corr_meetingDataService.GetAllCorr_meeting_with_id(customer.corr_meet_id).subscribe(data => this.Observ_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Observ_info) {
					console.log(item.corr_meet_id)
					this.Corr_meetingDataService.corr_meet_id = Number(item.corr_meet_id);
					console.log('testdepname', item.delay_emp_name);
					
					this.Corr_meetingDataService.corr_meet_date = item.corr_meet_date;
					this.Corr_meetingDataService.corr_meet_time = item.corr_meet_time;
					this.Corr_meetingDataService.corr_meet_loc = item.corr_meet_loc;

		
				};
				console.log('Component A is clicked!!', this.Corr_meetingDataService.corr_meet_id);
				this.Corr_meetingDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Corr_meeting, Corr_meetingDataService: Corr_meetingDataService) {
	
		console.log('CUSTOMER ID', customer.corr_meet_id);
		this.Corr_meetingDataService.deleteCorr_meeting(Number(customer.corr_meet_id)).subscribe(res => {
			this.get_corr_meet(Corr_meetingDataService);
			alert(res.toString());
