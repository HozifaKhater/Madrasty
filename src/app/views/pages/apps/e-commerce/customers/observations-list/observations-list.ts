...
export class ObservationListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'observer_id', 'lev_name', 'class_name', 'emp_name', 'observ_loc','observe_date','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_observation(ObservationsDataService);

	}
	get_observation(ObservationsDataService: ObservationsDataService) {

		this.ObservationsDataService.GetAllObservations().subscribe(data => this.ELEMENT_DATA = data,
	...
	Observ_info: any [];
	editCustomer(customer: Observations, ObservationsDataService: ObservationsDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.ObservationsDataService.GetAllObservations_with_id(customer.observer_id).subscribe(data => this.Observ_info = data,
			...
			() => {
				...
				console.log('Component A is clicked!!', this.ObservationsDataService.observer_id);
	...
	deleteCustomer(customer: Observations, ObservationsDataService: ObservationsDataService) {
	
		console.log('CUSTOMER ID', customer.observer_id);
		this.ObservationsDataService.deleteObservations(Number(customer.observer_id)).subscribe(res => {
			this.get_observation(ObservationsDataService);
			alert(res.toString());
