...
export class StudentPremListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'student_prem_id', 'prem_stu_name', 'prem_level', 'prem_class', 'prem_date', 'prem_leave_time','prem_arrive_time','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_studentprem(Student_premDataService);

	}
	get_studentprem(Student_premDataService: Student_premDataService) {

		this.Student_premDataService.GetAllStudent_prem().subscribe(data => this.ELEMENT_DATA = data,
		...
		this.Student_premDataService.GetAllStudent_prem_with_id(customer.student_prem_id).subscribe(data => this.Student_prem_info = data,
			...
			() => {
				for (let item of this.Student_prem_info) {
					...
					console.log('testdepname', item.prem_stu_name);
				...
				console.log('Component A is clicked!!', this.Student_premDataService.student_prem_id);
	...
	deleteCustomer(customer: Student_prem, Student_premDataService: Student_premDataService) {
		...
		console.log('CUSTOMER ID', customer.student_prem_id);
