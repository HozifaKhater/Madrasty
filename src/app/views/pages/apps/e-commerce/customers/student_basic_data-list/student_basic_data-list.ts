...
@Component({
	...
	selector: 'kt-student_basic_data-list',
    templateUrl: './student_basic_data-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class student_basic_dataListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'delay_id', 'delay_date', 'time_attend','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		private student_basic_dataDataService: student_basic_dataDataService
	) {
		this.get_delays(student_basic_dataDataService);

	}
	get_delays(student_basic_dataDataService: student_basic_dataDataService) {

		this.student_basic_dataDataService.GetAllstudent_basic_data().subscribe(data => this.ELEMENT_DATA = data,
	...
	Delays_info: any [];
    editCustomer(student_basic_data: student_basic_data, student_basic_dataDataService: student_basic_dataDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
        console.log('CUSTOMER ID', student_basic_data);
        this.student_basic_dataDataService.ser = student_basic_data.ser
        this.student_basic_dataDataService.GetAll_student_basic_data_with_id(student_basic_data.ser).subscribe(data => this.Delays_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Delays_info) {
					
					
                    
                    this.student_basic_dataDataService.student_id = item.student_id;
                    this.student_basic_dataDataService.student_name = item.student_name;
                    this.student_basic_dataDataService.previous_school = item.previous_school;
                    this.student_basic_dataDataService.address = item.address;
                    this.student_basic_dataDataService.transition_between_schools = item.transition_between_schools;
                    this.student_basic_dataDataService.foreign_schools = item.foreign_schools;
                    this.student_basic_dataDataService.student_history_notes = item.student_history_notes;
                    this.student_basic_dataDataService.health_status = item.health_status;

		
				};
				console.log('Component A is clicked!!', this.student_basic_dataDataService.ser);
				this.student_basic_dataDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
    deleteCustomer(student_basic_data: student_basic_data, student_basic_dataDataService: student_basic_dataDataService) {
	
        console.log('CUSTOMER ID', student_basic_data.ser);
        this.student_basic_dataDataService.deletestudent_basic_data(Number(student_basic_data.ser)).subscribe(res => {
			this.get_delays(student_basic_dataDataService);
			alert(res.toString());
