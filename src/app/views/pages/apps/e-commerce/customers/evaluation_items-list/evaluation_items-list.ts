...
import { Evaluation_itemsMaster, Evaluation_items } from '../../../../../../Evaluation_itemsMaster.Model';
...
@Component({
	...
	selector: 'kt-Evaluationitems-list',
	templateUrl: './evaluation_items-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class EvaluationItemsListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'evaluation_id', 'evaluation_object_name', 'evaluation_subject', 'evaluation_item_name', 'evaluation_appreciation','evaluation_score', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Evaluation_itemsMaster: Evaluation_itemsMaster[];
	...
	constructor(
		...
		this.get_Evaluationitems(Evaluation_itemsDataService);

	}
	get_Evaluationitems(Evaluation_itemsDataService: Evaluation_itemsDataService) {
	...
	evaluation_item_info: any [];
	editCustomer(customer: Evaluation_items, Evaluation_itemsDataService: Evaluation_itemsDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer.evaluation_item_id);
		this.Evaluation_itemsDataService.GetAllEvaluation_items_with_id(customer.evaluation_id).subscribe(data => this.evaluation_item_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.evaluation_item_info) {
					console.log(item.Evaluation_item_id)	
					this.Evaluation_itemsDataService.evaluation_id = Number(item.evaluation_id);
					console.log('testdepname', item.Evaluation_item_name);
					
				
					this.Evaluation_itemsDataService.evaluation_object = item.evaluation_object;
					this.Evaluation_itemsDataService.evaluation_object_name = item.evaluation_object_name;
					this.Evaluation_itemsDataService.evaluation_subject = item.evaluation_subject;
					this.Evaluation_itemsDataService.evaluation_subject_id = item.evaluation_subject_id;
					this.Evaluation_itemsDataService.evaluation_item_id = item.evaluation_item_id;
					this.Evaluation_itemsDataService.evaluation_item_name = item.evaluation_item_name;
					this.Evaluation_itemsDataService.evaluation_appreciation = item.evaluation_appreciation;
					this.Evaluation_itemsDataService.evaluation_score = item.evaluation_score;

	
				};
				console.log('Component A is clicked!!', this.Evaluation_itemsDataService.evaluation_item_name);
				this.Evaluation_itemsDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Evaluation_items, Evaluation_itemsDataService: Evaluation_itemsDataService) {
	
		console.log('CUSTOMER ID', customer.evaluation_id);
		this.Evaluation_itemsDataService.deleteEvaluation_items(Number(customer.evaluation_id)).subscribe(res => {
			this.get_Evaluationitems(Evaluation_itemsDataService);
			alert(res.toString());
