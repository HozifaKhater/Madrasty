...
import { Change_branchDataService } from '../../../../../../Services/Change_branchDataService';

import { Change_branchMaster, Change_branch } from '../../../../../../Change_branchMaster.Model';
...
@Component({
	...
	selector: 'kt-change-branch-list',
	templateUrl: './change_branch-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class ChangebranchListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'branch_id', 'branch_student', 'branch_level', 'branch_from','branch_to', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	Change_branchMaster: Change_branchMaster[];
	...
	constructor(
		...
		private Change_branchDataService: Change_branchDataService
	) {
		this.get_branch(Change_branchDataService);

	}
	get_branch(Change_branchDataService: Change_branchDataService) {

		this.Change_branchDataService.GetAllChange_branch().subscribe(data => this.ELEMENT_DATA = data,
	...
	Branch_info: any [];
	editCustomer(customer: Change_branch, Change_branchDataService: Change_branchDataService ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('CUSTOMER ID', customer);
		this.Change_branchDataService.GetAllChange_branch_with_id(customer.branch_id).subscribe(data => this.Branch_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.Branch_info) {
					console.log(item.branch_id)
					this.Change_branchDataService.branch_id = Number(item.branch_id);
					console.log('testdepname', item.branch_level);
					
					this.Change_branchDataService.branch_level = item.branch_level;
					this.Change_branchDataService.branch_class = item.branch_class;
					this.Change_branchDataService.branch_student = item.branch_student;
					this.Change_branchDataService.branch_from = item.branch_from;
					this.Change_branchDataService.branch_to = item.branch_to;
		
				};
				console.log('Component A is clicked!!', this.Change_branchDataService.branch_id);
				this.Change_branchDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: Change_branch, Change_branchDataService: Change_branchDataService) {
	
		console.log('CUSTOMER ID', customer.branch_id);
		this.Change_branchDataService.deleteChange_branch(Number(customer.branch_id)).subscribe(res => {
			this.get_branch(Change_branchDataService);
			alert(res.toString());
