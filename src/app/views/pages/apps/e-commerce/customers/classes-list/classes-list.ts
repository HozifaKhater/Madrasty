...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		this.get_classes(ClassesDataService);

	}
	get_classes(ClassesDataService: ClassesDataService) {
	...
	Classes_info: any [];
	editCustomer(customer: Classes, ClassesDataService: ClassesDataService ) {
		...
		this.ClassesDataService.GetAllClasses_with_id(customer.class_id).subscribe(data => this.Classes_info = data,
			...
			() => {
				...
				console.log('Component A is clicked!!', this.ClassesDataService.class_id);
	...
	deleteCustomer(customer: Classes, ClassesDataService: ClassesDataService) {
	
		console.log('CUSTOMER ID', customer.class_id);
